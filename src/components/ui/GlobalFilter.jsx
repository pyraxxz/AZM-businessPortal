...
export function GlobalFilter({ filters, values, onChange }) {
  ...
  return (
    <div
      ref={ref}
      className="flex items-center gap-2 px-4 py-2.5 rounded-xl border border-[var(--sn-border)] mb-5"
      style={{ background: 'var(--az-card)' }}
    >
      <div className="flex items-center gap-2 text-[var(--sn-text-muted)]">
        <Filter className="w-3.5 h-3.5" />
        <span className="text-[11px] font-bold uppercase tracking-wider">Filters</span>
        {activeCount > 0 && (
          <span className="px-1.5 py-0.5 rounded-full bg-[var(--sn-purple-subtle)] text-[var(--sn-purple)] text-[10px] font-bold">
            {activeCount}
          </span>
        )}
      </div>
          ...
          <button
            ...
            className={cn(
              ...
              values[filter.key] && values[filter.key] !== 'all'
                ? 'text-[var(--sn-purple)] bg-[var(--sn-purple-subtle)]'
                : 'text-[var(--sn-text-muted)] hover:bg-[var(--sn-border)]'
            )}
          >
            {filter.icon && <filter.icon className="w-3 h-3" />}
            <span>{filter.label}</span>
            {values[filter.key] && values[filter.key] !== 'all' && (
              <span className="text-[var(--sn-text-muted)]">: {filter.options?.find(o => o.value === values[filter.key])?.label || values[filter.key]}</span>
            )}
            <ChevronDown className="w-3 h-3" />
          </button>

          {open === filter.key && (
            <div className="absolute top-full left-0 mt-1 min-w-[180px] rounded-xl border border-[var(--sn-border)] shadow-2xl z-50 overflow-hidden" style={{ background: 'var(--az-card)' }}>
                ...
                <button
                  ...
                  className={cn(
                    ...
                    values[filter.key] === opt.value
                      ? 'bg-[var(--sn-purple-subtle)] text-[var(--sn-purple)]'
                      : 'text-[var(--sn-text-muted)] hover:bg-[var(--sn-border)]'
                  )}
                >
                  {opt.label}
      ...
      {activeCount > 0 && (
        <button
          onClick={() => onChange({})}
          className="ml-auto flex items-center gap-1 px-2 py-1 rounded-lg text-[11px] text-[var(--sn-text-muted)] hover:text-[var(--sn-red)] hover:bg-[#f43f5e10] transition-colors"
        >
          <X className="w-3 h-3" />
          Clear all
        </button>
