...
export function useStorefront() {
  ...
  const loadAll = async () => {
    ...
    try {
      const [draftData, publishedData, themesData, widgetsData, eligibilityData] = await Promise.all([
        storefrontApi.getDraftLayout().catch(() => null),
        storefrontApi.getPublished().catch(() => null),
        storefrontApi.listThemes().catch(() => []),
        storefrontApi.listWidgets().catch(() => []),
        storefrontApi.checkEligibility().catch(() => null),
      ]);
      setDraft(draftData); setPublished(publishedData);
      setThemes(themesData || []); setWidgets(widgetsData || []);
  ...
  const saveDraft = useCallback(async (layoutJson, themeId) => {
    ...
    try {
      const updated = await storefrontApi.saveDraftLayout(layoutJson, themeId, draft?.updatedAt);
  ...
  const publish = useCallback(async () => {
    setSaving(true); setError(null);
    try {
      const result = await storefrontApi.publishLayout();
      setPublished(result); return result;
  ...
  const scheduleAutoSave = useCallback((layoutJson, themeId) => {
    ...
    autoSaveTimer.current = setTimeout(() => { saveDraft(layoutJson, themeId).catch(() => {}); }, 30000);
  }, [saveDraft]);
  ...
  const addTile = useCallback((widgetType, defaultProps) => {
    if (!draft) return;
    const maxRow = draft.layoutJson?.tiles?.reduce((max, t) => Math.max(max, t.position.row + t.position.rowSpan), 0) ?? 0;
    const newTile = {
    ...
    const updated = { ...draft, layoutJson: { ...draft.layoutJson, tiles: [...(draft.layoutJson?.tiles ?? []), newTile] } };
    setDraft(updated); scheduleAutoSave(updated.layoutJson, draft.themeId);
  }, [draft, scheduleAutoSave]);

  const updateTile = useCallback((tileId, newProps) => {
    if (!draft) return;
    const updated = { ...draft, layoutJson: { ...draft.layoutJson,
      tiles: draft.layoutJson.tiles.map(t => t.id === tileId ? { ...t, props: { ...t.props, ...newProps } } : t) } };
    setDraft(updated); scheduleAutoSave(updated.layoutJson, draft.themeId);
  }, [draft, scheduleAutoSave]);

  const removeTile = useCallback((tileId) => {
    if (!draft) return;
    const updated = { ...draft, layoutJson: { ...draft.layoutJson, tiles: draft.layoutJson.tiles.filter(t => t.id !== tileId) } };
    setDraft(updated); scheduleAutoSave(updated.layoutJson, draft.themeId);
  }, [draft, scheduleAutoSave]);
  ...
  const applyTemplate = useCallback(async (templateId) => {
    setSaving(true);
    try { const newDraft = await storefrontApi.applyTemplate(templateId); setDraft(newDraft); }
    catch (err) { setError(err.message); throw err; } finally { setSaving(false); }
  }, []);

  const revertToVersion = useCallback(async (versionId) => {
    setSaving(true);
    try { const newDraft = await storefrontApi.revertToVersion(versionId); setDraft(newDraft); }
    catch (err) { setError(err.message); throw err; } finally { setSaving(false); }
  }, []);
